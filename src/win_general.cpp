...
void WinGenSDL::initWin( const int & pos_x, const int & pos_y ) {
	if ( win == nullptr ) {
		...
		win = SDL_CreateWindow( "Obstacle Box", pos_x, pos_y, width, hight, wFlags );
...
WinGenSDL & WinGenSDL::operator=(WinGenSDL & wgs) {
	...
	win = SDL_CreateWindow("ObstacleBoxWin_test", 0, 0, width, hight, SDL_WINDOW_OPENGL );
...
bool WinGenSDL::test() {
	...
	if( width != 0 && hight != 0 && width == a && hight == b )
	...
	t_wgs1.initWin( 4, 9 );	
	t_wgs2 = t.wgs1;
	if ( a == t_wgs2.get_win_width && b == t_wgs2.get_win_hight )
